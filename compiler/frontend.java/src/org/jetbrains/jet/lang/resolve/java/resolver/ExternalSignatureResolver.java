...
public class ExternalSignatureResolver {
    ...
    public static class MethodSignature extends MemberSignature {
    ...
    public static class FieldSignature extends MemberSignature {
        private final JetType returnType;

        public FieldSignature(@NotNull JetType returnType, @Nullable String signatureError) {
            super(signatureError);
            this.returnType = returnType;
        }
    ...
    @NotNull
    public MethodSignature resolveAlternativeMethodSignature(
            @NotNull JavaMethod method,
    ...
    @SuppressWarnings("MethodMayBeStatic")
    @NotNull
    public FieldSignature resolveAlternativeFieldSignature(@NotNull JavaField field, @NotNull JetType returnType, boolean isVar) {
        AlternativeFieldSignatureData data = new AlternativeFieldSignatureData(field, returnType, isVar);

        if (data.isAnnotated() && !data.hasErrors()) {
            return new FieldSignature(data.getReturnType(), null);
        }

        String error = data.hasErrors() ? data.getError() : null;
        return new FieldSignature(returnType, error);
    }
